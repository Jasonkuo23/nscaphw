...
class Setting:
    ...
    def __init__(
        ...
        if max_collision_wait_time is None:
            self.max_collision_wait_time = coefficient * self.packet_time * self.host_num
            # TODO: this is not complete, change this (hint: add two parameters)
        else:
            self.max_collision_wait_time = max_collision_wait_time

        if p_resend is None:
            self.p_resend = round(1 / (coefficient * self.host_num), 2) # TODO: this is not complete, change this (hint: add one parameters)
        else:
