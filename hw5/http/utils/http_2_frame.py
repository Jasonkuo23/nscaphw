...
class Frame:
    ...
    def to_bytes(self):
        ...
        bytes = struct.pack(f"!L", (self.length << 8) | self.type)
        bytes += struct.pack(f"!B", self.flags)
        bytes += struct.pack(f"!L", (self.r << 31) | self.stream_id)
        bytes += self.payload
        return bytes
...
def bytes_to_frames(data):
    ...
    while len(data) > 0:
        ...
        if len(data) >= 9+length:
            ...
            flags = data[4]
            r_stream_id, = struct.unpack(f"!L", data[5:9])
